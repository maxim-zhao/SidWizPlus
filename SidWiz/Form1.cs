...
namespace SidWiz {
    public partial class Form1 : Form
    {
            ...
            string logo, string vgmFile)
        {
            ...
            var baseImage = new Bitmap(width, height, PixelFormat.Format24bppRgb);
            int marginBottom = 0;
            using (var g = Graphics.FromImage(baseImage))
            {
                ...
                if (logo != null)
                {
                    ...
                    {
                        g.DrawImage(bm, width - bm.Width, height - bm.Height, bm.Width, bm.Height);
                    }
                }

                if (vgmFile != null)
                {
                    using (var f = new Font("Candara"/*"Modern TwoSxtn ITC Std Light"*/, 16, FontStyle.Regular))
                    {
                        g.TextRenderingHint = TextRenderingHint.AntiAlias;
                        var info = GetTagInfo(vgmFile);
                        if (info.Length > 0)
                        {
                            // Measure it
                            var size = g.MeasureString(info, f, new SizeF(width, height));
                            // Draw it at the bottom
                            g.DrawString(info, f, Brushes.White, new RectangleF(0, height - size.Height, width, height));
                            marginBottom = (int) size.Height;
                        }
                    }
                }
            }
            ...
            var renderer = new WaveformRenderer()
            {
                ...
                RenderingBounds = new Rectangle(0, 0, width, height - marginBottom)
            };

            foreach (var channel in voiceData)
            {
                renderer.AddChannel(channel, Color.White, 3, "");
            }
            //using (var output = new FfmpegOutput(_ffPath, filename, width, height, fps, ffOutArgs.Text, filenames))
            using (var preview = new PreviewOutput(1))
            {
        ...
        private static string GetTagInfo(string vgmFile)
        {
